...
**Status:** Version from 2021 (v1.0) is stable.
Version from 2026 has updated ESP-IDF SDK and Arduino core,
  but DCC signal generation suffers from occasional timing glitches,
  so it not as reliable as the old one.
...
The main goal is to remake the PCB for a DIN rail enclosure,
  ...
  (they were one shot before) and upgrading thecodebase to
