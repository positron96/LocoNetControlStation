...
[NOTE]
--
JMRI sends this message to command turnout in case "Bypass Bushby Bit" flag is set in the Command station.

With this flag set, normal turnout commands(<<OPC_SW_REQ>>) are not forwarded to DCC by command stations.

See details
https://www.jmri.org/help/en/html/scripthelp/LnBushbyForwarder/LnBushbyForwarder.shtml[here] and
https://www.jmri.org/jython/LnBushbyForwarder.py[here (a python script to turn normal commands to bushby ones)].

Upon receiving, JMRI treats this OPCode equally to <<OPC_SW_REQ>>.
--


|[[OPC_SW_STATE]]OPC_SW_STATE |0xBC | Request state of SWITCH | <<OPC_LACK>>
| 3+a| `<0xBC>,<SW1>,<SW2>,<CHK>`

Request to a switch to report its state.

<SW1> and <SW2> are same as in <<OPC_SW_ACK>> (only address bits are used)
...
* IF SRC=DEST (NULL move) then SRC(=DEST) slot state is set to IN_USE (if move is legal).
* If DEST=0 (DISPATCH Put), mark slot as DISPATCH, return <<OPC_SL_RD_DATA,slot status>> `<0xE7>` of destination slot <DEST> (if move is legal).

NOTE: Probably <SRC> slot needs to be returned if DEST=0.
...
[NOTE]
--
JMRI sends this message to tell turnout to change its state if its "Bypass Bushby Bit" is not set
...
if STAT2.2=1, the STAT2.3=0 means EX1/EX2 are ALIAS

ID1/ID2 are two 7 bit values encoding a 14 bit unique DEVICE usage ID:

[horizontal]
 00/00:: means NO ID being used
 01/00 to 7F/01:: slot is used by PC. Low nibble is typically a PC number (PC can use high values)
 00/02 to 7F/03:: SYSTEM reserved
 00/04 to 7F/7E:: normal range for throttles
...
[NOTE]
--
This message is used to work with System Variables (SVs).

This is described in LocoNet Personal Use Extension (see e.g.
...
NOTE: This could be STAT2.3 instead of STAT2.4 (as STAT2.4 is not mentioned anywhere, while STAT2.3 seems to mean "treat ID fields as ID")
...
Broadcast LocoNet Switch adr is then `<SW1>=<0,1,1,1-1,0,c,b>`, `<SW2>=<0,0,a,D-1,1,1,1>`
...
Interrogate LocoNet Switch adr is `<SW1>= <0,1,1,1-1,0,c,b>`, `<SW2>=<0,0,a,1-0,1,1,1>`.

[NOTE]It should probably be `<SW2>=<0,0,a,0-0,1,1,1>` as this is how JMRI interrogates devices on boot.
...
Use <<OPC_WR_SL_DATA,<EF> WR_SL_DATA>> to write new clock information;
Request to read slot 0x7B (<<OPC_RQ_SLOT_DATA, <BB><7B>..>>),
  will return current System clock information, and other throttles will update to this SYNC.
Note that all attached display devices keep a current clock calculation based on this SYNC read value,
  i.e. devices MUST not continuously poll the clock SLOT to generate time,
  but use this merely to restore SYNC and follow current RATE etc.
This clock slot is typically "pinged" or read SYNC'd every 70 to 100 seconds,
  by a single user, so all attached devices can synchronise any phase drifts.
Upon seeing a SYNC read, all devices should reset their local
...
[NOTE]
--
for `256-MINS_60` and `256-HRS_24`,
JMRI has formula (LocoNetSlot.java):
```
MINS = (255 - (60 - minutes)) & 0x7F
HRS = (256 - (24 - hours)) & 0x7F
```

LocoNet library has the formulas:
```
MINS = minutes + 67
HRS = hours + 104
```
