...
|OPC_LOCO_ADR |0xBF |REQ loco ADR |YES <E7><<SLOT_READ>>
4+| `<0xBF>,<0>,<ADR>,<CHK>` 
...
4+| ;<0xBD>,<SW1>,<SW2>,<CHK> REQ SWITCH function
...
4+| ;<0xBC>,<SW1>,<SW2>,<CHK> REQ state of SWITCH


|OPC_RQ_SL_DATA |0xBB |Request SLOT DATA/status block |YES <E7><<SLOT_READ>>
4+a| <0xBB>,<SLOT>,<0>,<CHK> Request SLOT DATA/status block


|OPC_MOVE_SLOTS |0xBA |MOVE slot SRC to DEST |YES <E7><<SLOT_READ>> or <<LACK>>
4+a|
`<0xBA>,<SRC>,<DEST>,<CHK>` 
...
4+| ;<0xB9>,<SL1>,<SL2>,<CHK> SLAVE slot SL1 to slot SL2
...
4+|;<0xB1>,<SN1>,<SN2>,<CHK> SENSOR state REPORT
...
4+|;<0xB0>,<SW1>,<SW2>,<CHK> REQ SWITCH function
<SW1> =<0,A6,A5,A4- A3,A2,A1,A0>, 7 ls adr bits. A1,A0 select 1 of 4 input pairs in a DS54
<SW2> =<0,0,DIR,ON- A10,A9,A8,A7> Control bits and 4 MS adr bits.
,DIR=1 for Closed,/GREEN, =0 for Thrown/RED
,ON=1 for Output ON, =0 FOR output OFF
...
4+a| NOTE: CODES 0xA8 to 0xAF have responses
...
|[[SLOT_READ]]OPC_SL_RD_DATA |0xE7 | SLOT DATA return, 10 bytes |NO
...
4+| ;<0xE5>,<10>,<SRC>,<DSTL><DSTH>,<PXCT1>,<D1>,<D2>,<D3>,<D4>,
...
4+| ;<0xED>,<0B>,<7F>,<REPS>,<DHI>,<IM1>,<IM2>,<IM3>,<IM4>,<IM5>,<CHK>
