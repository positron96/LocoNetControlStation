= LocoNet(R) Personal Use Edition 1.0 SPECIFICATION
...
may change the SLOT to IN_USE by performing a NULL MOVE instruction, opcode (<<MOVE_SLOTS,OPC_MOVE_SLOTS>>
...
If address is not found, master puts address in free slot and sends <<SL_RD_DATA>>`<E7>......`

If there is no free slot, <<LACK>> with argument 0 is returned (`<B4>,<3F>,<0>,<CHK>`).
 ...
 * <<LACK, <0xB4><3D><00><CHK> >> if DCS100 FIFO is full, command rejected
 * <<LACK, <0xB4><3D><7F><CHK> >> if DCS100 accepted

NOTE: JMRI treats this OPCode equally to <<SW_REQ>>
...
<SW1> and <SW2> are same as in <<SW_ACK, OPC_SW_ACK>>
...
* If DEST=0 (DISPATCH Put), mark slot as DISPATCH, return <<SL_RD_DATA,slot status>> `<0xE7>` of destination slot <DEST> if move legal. 
...
Reply is return of <<SL_RD_DATA, Slot status>> <0xE7>. 
...
Immediate response of <<LACK>> `<0xB4><30><00>` if command failed, otherwise no response.
...
NOTE: JMRI treats this message as equal to <<SW_ACK,OPC_SW_ACK>>
...
Use <<WR_SL_DATA,<EF> WR_SL_DATA>> to write new clock information; Request to read slot 0x7B (<<RQ_SLOT_DATA, <BB><7B>..>>), will return current System clock information, and other throttles will update to this SYNC. Note that all attached display devices
